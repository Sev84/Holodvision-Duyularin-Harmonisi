...
const tasks = [
  ...
  "sosyal medya gönderi metnini kopyalayın veya güvenli paylaşım ekranını açın.",
];
...
function records() {
  try { return JSON.parse(localStorage.getItem("duyu-sense-usability") || "[]"); }
  catch { return []; }
}
...
function formRecord() {
  ...
  return {
    schema: "duyu-sense-usability-v1",
...
form.addEventListener("submit", (event) => {
  ...
  current.push(record);
  localStorage.setItem("duyu-sense-usability", JSON.stringify(current));
...
document.querySelector("#export-json").addEventListener("click", () => {
  ...
  download(JSON.stringify({ exportedAt: new Date().toISOString(), records: current }, null, 2), "HARMONY-SENSE-Usability-Tests.json", "application/json");
...
document.querySelector("#export-csv").addEventListener("click", () => {
  ...
  download(`\uFEFF${csv}`, "HARMONY-SENSE-Usability-Summary.csv", "text/csv;charset=utf-8");
...
document.querySelector("#clear-tests").addEventListener("click", () => {
  ...
  localStorage.removeItem("duyu-sense-usability");
